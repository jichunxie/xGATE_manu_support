---
title: "Pancreas_all_methods"
output: html_document
date: "2025-02-11"
---

ORA Pathway Analysis T1D
...
set.seed(1234)
pancreas_data = readRDS("/work/of21_work/pancreas_human.rds")
head(pancreas_data@meta.data)
metadata = pancreas_data@meta.data

pancreas_cells_t1d = metadata[metadata$cell_type == "type B pancreatic cell" & metadata$disease_state == "T1D",]

count_matrix <- pancreas_data@assays[["RNA"]]@counts

pancreas_count_matrix = count_matrix[, rownames(pancreas_cells_t1d)]

# Create a new Seurat object
so <- CreateSeuratObject(counts = pancreas_count_matrix)

# Run SCTransform
so <- SCTransform(object = so, vst.flavor = "v2", verbose = FALSE)

# Get the filtered count matrix
pancreas_count_matrix <- so@assays[["SCT"]]@counts
...
genes <- rownames(pancreas_count_matrix)
...
pancreas_count_matrix <- pancreas_count_matrix[rownames(pancreas_count_matrix) %in% updated_genes$ensembl_gene_id, ]
...
rownames(pancreas_count_matrix) <- gene_mapping[rownames(pancreas_count_matrix)]

# Verify the updated row names
head(rownames(pancreas_count_matrix))
...
cutoff <- quantile(rowMeans(pancreas_count_matrix), 0.95)
highly_expressed_genes <- rownames(pancreas_count_matrix)[rowMeans(pancreas_count_matrix) > cutoff]
...
enriched_in_pancreas <- enriched_pathways$Description %in% pancreas_pathways

print(enriched_in_pancreas)


enriched_pathways <- as.data.frame(result)[, c("Description", "pvalue", "qvalue")]

# Determine which pancreas pathways are enriched
enrichment_status <- pancreas_pathways %in% enriched_pathways$Description

# Create a data frame that has pathway names and their enrichment status
final_results <- data.frame(Pathway = pancreas_pathways,
...
write.csv(final_results_csv, "/work/of21_work/pancreas_ORA_pathways_t1d.csv", row.names = FALSE)
...
pancreas_cells_ctrl = metadata[metadata$cell_type == "type B pancreatic cell" & metadata$disease_state == "Control",]

count_matrix <- pancreas_data@assays[["RNA"]]@counts

pancreas_count_matrix = count_matrix[, rownames(pancreas_cells_ctrl)]
...
# Write the new data frame to a CSV file without row names
write.csv(final_results_csv, "/work/of21_work/pancreas_ORA_pathways_ctrl.csv", row.names = FALSE)
...
# Update the rownames of 'pancreas_count_matrix' using the mapping
...
filtered_df <- results_df[results_df$PathwayName %in% pancreas_pathways, ]
...
# Write the new data frame to a CSV file without row names
write.csv(final_results_csv, "/work/of21_work/pancreas_AUCell_pathways_t1d.csv", row.names = FALSE)


```

AUCell Pathway Analysis Ctrl
```{r}
library(biomaRt)
library(clusterProfiler)
library(AUCell)
...
# Write the new data frame to a CSV file without row names
write.csv(final_results_csv, "/work/of21_work/pancreas_AUCell_pathways_ctrl.csv", row.names = FALSE)


```


scGSEA Pathway Analysis T1D
```{r}
library(Seurat)
library(dplyr)
library(tidyverse)
...
# Get the filtered count matrix
pancreas_count_matrix <- so@assays[["SCT"]]@counts
count_filtered = pancreas_count_matrix
# ----------------------------
# Step 2: Gene Mapping
# ----------------------------
...
updated_genes <- converted_genes[which(is.na(converted_genes$entrezgene_id)==FALSE), ]
# Filter rows of pancreas_count_matrix
count_filtered <- count_filtered[rownames(count_filtered) %in% updated_genes$ensembl_gene_id, ]
...
pancreas_pathways = c("Autophagy",
    ...
    "Thyroid cancer",
    "Shigellosis")
# Filter the list to only include entries with names in pancreas_pathways
filtered_pathway_genes_list <- pathway_genes_list[names(pathway_genes_list) %in% pancreas_pathways]
...
#writeMM(data[["scgsea"]][["x"]], "/work/of21_work/pancreas_scgsea_april_30.mtx")
...
t1d_active_pathways = c("Apoptosis", "Cytokine-cytokine receptor interaction", "FoxO signaling pathway")

t1d_inactive_pathways = c("Salmonella infection",
    "Bacterial invasion of epithelial cells",
    "Pathogenic Escherichia coli infection",
    "Thyroid cancer",
    "Shigellosis",
...
# Identify pathways in pancreas_pathways that are not in medians_df
#missing_pathways <- setdiff(pancreas_pathways, colnames(medians_df))
...
# (Optional) Reorder the columns so that they follow the order of pancreas_pathways.
# This is helpful if you want a consistent column order.
# Combine the pathway lists
desired_pathways <- c(t1d_active_pathways, t1d_inactive_pathways)

# Identify pathways that are missing in medians_df
missing_pathways <- setdiff(desired_pathways, colnames(medians_df))

# Add missing pathways as columns filled with 0
if(length(missing_pathways) > 0) {
  medians_df[missing_pathways] <- 0
}

# Now subset medians_df to include only the desired pathways (in the specified order)
medians_df <- medians_df[, desired_pathways, drop = FALSE]
...
ground_truth <- sapply(colnames(medians_df), function(pathway) {
  if (pathway %in% t1d_active_pathways) {
    return(1)
  } else if (pathway %in% t1d_inactive_pathways) {
    return(0)
  }
})
...
thresholds <- seq(min(score_values[score_values > 0]), max(score_values), length.out = 100)
...
# Create the final data frame using pancreas_pathways:
final_medians <- data.frame(
  Pathway.Name = c(t1d_active_pathways, t1d_inactive_pathways),
...
write.csv(final_medians, "/work/of21_work/pancreas_scGSEA_pathways_t1d.csv", row.names = FALSE)
...
medians_df <- medians_df[, c(ctrl_active_pathways, ctrl_inactive_pathways), drop = FALSE]
...
# and 0 (inactive) if in cancer_pathways.
ground_truth <- sapply(colnames(medians_df), function(pathway) {
  if (pathway %in% ctrl_active_pathways) {
    return(1)
  } else if (pathway %in% ctrl_inactive_pathways) {
    return(0)
  }
})
...
# Create the final data frame using pancreas_pathways:
final_medians <- data.frame(
  Pathway.Name = c(ctrl_active_pathways, ctrl_inactive_pathways),
  Type = "scGSEA", # all rows get "scGSEA"
  Group = "Ctrl" ,
  stringsAsFactors = FALSE
)
...
write.csv(final_medians, "/work/of21_work/pancreas_scGSEA_pathways_ctrl.csv", row.names = FALSE)
